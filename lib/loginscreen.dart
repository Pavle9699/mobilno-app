...
import 'package:prisustvo/homescreen.dart';
...
class _LoginScreenState extends State<LoginScreen> {
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                fieldTitle("Studentski ID"),
                customField("Unesite vas username", idController,false),
                fieldTitle("Sifra"),
                customField("Unesite vasu sifru", passController,true),
                GestureDetector(
                  onTap: () async{
                    ...
                    if(id.isEmpty){
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                        content: Text(
                          "Polje Studentski ID je prazno"
                        ),
                      ));
                    } else if (password.isEmpty){
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                        content: Text(
                            "Polje password je prazno"
                        ),
                      ));
                    } else{
                      ...
                      try{
                        ...
                        if(e.toString() == "RangeError (index): Invalid value: Valid value range is empty: 0"){
                          setState(() {
                            error = "Studentski ID ne postoji";
                          });
                        } else {
                          setState(() {
  ...
  Widget fieldTitle(String title) {
    return Container(
      ...
      child: Text(
        title,
        style: TextStyle(fontFamily: "NexaBold", fontSize: screenWidth / 24),
      ),
    );
  }
